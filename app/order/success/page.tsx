...
export default function OrderSuccessPage() {
  const { clearCart, isInitialized, items } = useCart();
  ...
  return (
    <div className="container mx-auto px-4 py-16 text-center font-avenir text-white">
      ...
      {sessionId && (
        <p className="text-sm text-white/60 mb-2">Order reference (Session ID): {sessionId}</p>
      )}
      {quoteId && (
        <p className="text-sm text-white/60 mb-8">
          <span className="text-[#5fe496]">Quote ID: {quoteId}</span><br/>
