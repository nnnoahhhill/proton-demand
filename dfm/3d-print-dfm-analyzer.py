...
DEFAULT_CONFIG = {
    ...
    # Thresholds
    "printability_threshold": 0.4,  # 0-1 scale of printability score (lowered to be more lenient)
...
class DFMAnalyzer:
    ...
    def find_slicer_path(self):
        ...
        env_slicer_path = os.environ.get('PRUSA_SLICER_PATH')
        if env_slicer_path:
            logger.info(f"Found PrusaSlicer path in environment variable: {env_slicer_path}")
            if os.path.exists(env_slicer_path) and os.access(env_slicer_path, os.X_OK):
                logger.info(f"Verified slicer at: {env_slicer_path}")
                return env_slicer_path
            else:
                logger.warning(f"PrusaSlicer path from environment variable ({env_slicer_path}) is not valid or executable")

        # Next check if it's in the PATH
    ...
    def analyze_model(self):
        ...
        # Run slicer analysis for print time and cost estimation if available
        if not self.stl_file:
            raise RuntimeError("STL file not available. Cannot perform print time and cost estimation.")

        # Try to use PrusaSlicer for accurate estimates if available
        if not self.config["slicer_path"]:
            # Try to find PrusaSlicer in PATH if not specified
            self.config["slicer_path"] = shutil.which("prusa-slicer")

        # Always use fallback method based on mesh analysis
        logger.info("Using fallback method for time and cost estimates.")
        self.run_fallback_analysis()
    ...
    def run_fallback_analysis(self):
        """Use a fallback method to estimate print time and material usage based on mesh statistics"""
        if not hasattr(self, 'mesh_stats'):
            raise RuntimeError("Mesh statistics not available for analysis")
        ...
        bbox_volume_mm3 = self.mesh_stats.get('bbox_volume_mm3', 0)

        # Calculate material usage based on volume and density
        # Default density for common materials (g/cm³)
        material_density = {
            'PLA': 1.24,
            'ABS': 1.04,
            'PETG': 1.27,
            ...
            'Resin': 1.1,
            'Nylon': 1.01,
            'DEFAULT': 1.2  # Default if material not known
        }

        # Get density for the selected material
        density = material_density.get(self.config['material_type'], material_density['DEFAULT'])

        # Calculate weight in grams (volume in mm³ to cm³, then multiply by density)
        weight_g = (volume_mm3 / 1000) * density

        # Estimate print time based on printer type and volume
        if self.config['printer_type'] == 'FDM':
            # FDM printing time calculation
            # Estimate based on volume and complexity
            # Typical FDM printer can process about 5-15 mm³/s depending on settings
            avg_volume_per_sec = 8  # mm³/s of filament extrusion for FDM
            complexity_factor = surface_area_mm2 / (volume_mm3**(2/3))  # Higher for complex parts
            complexity_factor = min(max(complexity_factor / 10, 1), 3)  # Normalize to 1-3 range

            # Adjust for infill percentage (default 20%)
            infill_factor = 0.8 + (0.2 * 0.2)  # 80% shell + 20% of 20% infill

            # Calculate print time in seconds
            print_time_sec = (volume_mm3 * infill_factor * complexity_factor) / avg_volume_per_sec

            # Add setup time and non-printing moves
            print_time_sec += 300  # 5 minutes base time

        elif self.config['printer_type'] == 'SLA':
            # SLA printing time calculation
            # Based on layer height and build volume height
            layer_height_mm = 0.05  # Typical SLA layer height
            z_height_mm = self.mesh_stats.get('bbox_size_z', 0)
            num_layers = z_height_mm / layer_height_mm

            # Each layer takes about 6-15 seconds depending on area
            avg_layer_time_sec = 10  # seconds per layer
            print_time_sec = num_layers * avg_layer_time_sec

            # Add setup and post-processing time
            print_time_sec += 600  # 10 minutes base time

        else:  # SLS or other
            # Generic calculation based on volume
            print_time_sec = volume_mm3 / 10  # Rough estimate
            print_time_sec += 1800  # 30 minutes base time for SLS

        # Convert to hours
        print_time_hours = print_time_sec / 3600

        # Calculate material cost
        # Material cost per kg/liter
        material_cost = {
            'PLA': 25,      # $/kg
            'ABS': 30,      # $/kg
            'PETG': 35,     # $/kg
        ...
        # Get cost for the selected material
        cost_per_unit = material_cost.get(self.config['material_type'], material_cost['DEFAULT'])

        # Calculate material cost
        if self.config['printer_type'] == 'SLA':
            # For SLA, convert weight to volume in liters
            volume_liters = volume_mm3 / 1000000
            material_cost = volume_liters * cost_per_unit
        else:
            # For FDM/SLS, use weight in kg
            weight_kg = weight_g / 1000
            material_cost = weight_kg * cost_per_unit
        ...
        hourly_rate = machine_cost_per_hour.get(self.config['printer_type'], machine_cost_per_hour['DEFAULT'])
        machine_cost = print_time_hours * hourly_rate

        # Calculate total cost with markup
        base_cost = material_cost + machine_cost
        setup_fee = 10  # Fixed setup fee
        markup_factor = 1.7  # 70% markup (50% + 20% additional)

        total_cost = (base_cost + setup_fee) * markup_factor

        # Ensure minimum order value
        minimum_order = 25  # Minimum order value
        total_cost = max(total_cost, minimum_order)

        # Round to nearest dollar
        total_cost = round(total_cost)

        # Store the results
        self.mesh_stats['print_time_hours'] = round(print_time_hours, 2)
        self.mesh_stats['material_weight_g'] = round(weight_g, 2)
        self.mesh_stats['material_cost'] = round(material_cost, 2)
        self.mesh_stats['machine_cost'] = round(machine_cost, 2)
        self.mesh_stats['total_cost'] = total_cost
        self.mesh_stats['currency'] = 'USD'

        logger.info(f"Fallback analysis complete: {print_time_hours:.2f} hours, {weight_g:.2f}g, ${total_cost}")
